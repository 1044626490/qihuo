...
class FiveMinutesChart extends React.Component{
    ...
    componentDidMount(){
        let chart = echarts.init(document.getElementById('five-minutes-chart'));
        ...
        let setI = setInterval(()=>{
            let dd = rawDatas[count-1];
            // rawData.push(dd);
            if(rawData.length < 50){
                rawData.push(dd)
            }else {
                rawData = rawData.slice(rawData.length-49,rawData.length);
                rawData.push(dd)
            }
            count++;
            let dates = rawData.map((item) => {
            ...
            console.log(chart,count,rawData)
        },1000)
    }

    calculateMA(dayCount, data) {
        let result = [];
        let num = data.length >= 50?data.length-49:0
    ...
    render(){
        return(
            <div id="five-minutes-chart" style={{width: "100%",height: "38vh"}}>
...
export default FiveMinutesChart